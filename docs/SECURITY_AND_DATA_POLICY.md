...
CatEx 自有代码与文档采用 SPDX 标识为 `BSD-3-Clause` 的标准 BSD 3-Clause License。第三方软件、VASP/POTCAR、论文/SI 和外部数据不因进入 CatEx 工作流或被文档引用而改变其原有许可；不确定来源或再分发权时必须排除在公开仓库之外。
...
- 取消作业、清理目录和删除大型文件必须由明确策略与人工授权触发。Web 工作台只实现了
  当前提交回执绑定的单 job `scancel`，要求独立确认并记录本地回执；它不修改或删除远端文件。
